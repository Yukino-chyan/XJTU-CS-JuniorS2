...
// AST 节点 / 符号表 / token / 语义结果等数据结构已抽到 ast.h（供 scanner、irgen、lab6 共用）
#include "ast.h"
...
// Lab6 接口：把“解析建 AST → 语义分析”打包成一次调用，供进程内复用
SemanticResult parse_and_analyze(const SLR1Result& slr, const vector<TokenRecord>& tokens)
{
    SemanticResult result = run_semantic(slr, tokens);   // 解析 + 建 AST
    analyze(result);                                     // 语义分析 + 符号表 + 类型检查
    return result;
}

#ifndef LAB6_BUILD   // 独立构建 semantic.exe 时保留 main；被 Lab6 链接时（-DLAB6_BUILD）排除
